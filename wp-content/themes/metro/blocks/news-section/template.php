...
$class_name = 'news-section mm-section';
...
$mode = get_field('mode');
$posts = MetroManhattanHelpers::get_posts_by_mode($mode, ['category' => $category, 'post_type' => 'post', 'numberposts' => 3]);
$range = get_field('range');
$button = get_field('button');
$is_centered = get_field('is_centered');
$border = get_field('border');

$class_name = ($is_centered) ? $class_name . ' not-home' : $class_name;
$class_name = ($border) ? $class_name . ' border' : $class_name;
$class_name .= ' '. $background;

?>
<section <?php echo $anchor; ?> data-target="swiper" data-swiper-slides="<?php echo $range ?>" data-swiper-tablet-slides="2"
                    ...
                    <div class="swiper-wrapper">
                        <?php foreach ($posts as $post): ?>
                            <div class="swiper-slide">
                                <?php get_template_part('templates/parts/blog', 'post', ['id' => $post, 'author' => true, 'date' => true, 'type_of_articles' => 'real-estate-insights']); ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
