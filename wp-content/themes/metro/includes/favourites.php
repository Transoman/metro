<?php
  ...
  class MMFavourites {
    ...
    public static function sync_saved_listings_with_zoho( $user_id ) {
      ...
      $listing_data = array_map( function ( $listing ) use ( $wpdb, $user_id ) {
        ...
        // Get the creation time from user meta (when it was favorited)
        $created_at = get_user_meta( $user_id, 'user_favorites_' . $listing_id . '_created_at', true );
        if ( empty( $created_at ) ) {
          $created_at = 'Unknown'; // Default value if not found
        }
    ...
    private static function add_favourite( $user_id, $listing_id ) {
      ...
      $adjusted_time       = $current_time - ( 5 * HOUR_IN_SECONDS );
      $adjusted_time_mysql = date( 'Y-m-d H:i:s', $adjusted_time );
      add_user_meta( $user_id, 'user_favorites_' . $listing_id . '_created_at', $adjusted_time_mysql );
    }
