<?php
  ...
  class MMEditUser {
    ...
    /**
     * Handles the AJAX request for deleting a user account.
     */
    public function delete_user() {
      ...
      if ( $user ) {
        ...
        if ( wp_delete_user( $user_id ) ) {
          error_log( 'User deleted successfully: ' . $user_details );
          $response['redirect_to'] = get_home_url();
        } else {
          error_log( 'Failed to delete user: ' . $user_details );
